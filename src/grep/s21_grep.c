...
void scan_files(int argc, char **argv, t_pattern *patterns, int patterns_cnt, int *flags) {
    int kFiles_cnt = cnt_files(argc, argv, patterns, flags);
    if (kFiles_cnt) {
        t_file files[kFiles_cnt];
        init_files(files, kFiles_cnt);
        get_files(argc, argv, patterns, files, flags);
        files_processing(patterns, patterns_cnt, files, kFiles_cnt, flags);
    } else {
...
void get_files(int argc, char **argv, t_pattern *patterns, t_file *files, int *flags) {
    int same = 0;
    int k = 0;
    for (int i = 1; i < argc; i++) {
        if (is_arg_a_file(i, argv, patterns, &same, flags)) files[k++].file_name = argv[i];
    }
}
